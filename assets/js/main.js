...
(function() {
    ...
    function init() {
        ...
        // Initialize developer version banner
        initVersionBanner();
    }

    function initVersionBanner() {
        // Only show if version info exists
        if (typeof window.WACTC_VERSION === 'undefined') return;

        const v = window.WACTC_VERSION;

        // Create banner element
        const banner = document.createElement('div');
        banner.id = 'dev-version-banner';
        banner.innerHTML = `
            <span id="dev-banner-toggle" title="Click to expand/collapse">🔧 v${v.commit}</span>
            <div id="dev-banner-details">
                <strong>Branch:</strong> ${v.branch}<br>
                <strong>Deployed:</strong> ${v.deployedAt}<br>
                <strong>Commit:</strong> ${v.message}
            </div>
            <button id="dev-banner-close" title="Hide banner">✕</button>
        `;
        ...
        style.textContent = `
            ...
            #dev-banner-details {
                display: none;
                color: #ccc;
                border-left: 1px solid rgba(168,230,163,0.3);
                padding-left: 8px;
            }
            #dev-banner-details.open {
                display: block;
            }
            #dev-banner-close {
                background: none;
        ...
        document.getElementById('dev-banner-toggle').addEventListener('click', function() {
            const details = document.getElementById('dev-banner-details');
            details.classList.toggle('open');
        });

        // Close button hides banner for this session
